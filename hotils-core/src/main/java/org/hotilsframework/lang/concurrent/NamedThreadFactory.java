 ...
 * @Description: TODO   创建线程工厂类，此线程可选配置：
 *          <pre>
 *              1. 自定义线程命名前缀
 *              2. 自定义是否守护线程
 *          </pre>
 * @Author: hireny
 * @Date: Created in 2020-01-10 23:13
 * @Version: 1.0
 */
public class NamedThreadFactory implements ThreadFactory {
    ...
    public NamedThreadFactory(String prefix, ThreadGroup threadGroup, boolean isDaemon, UncaughtExceptionHandler handler) {
        this.prefix = StringUtils.isBlank(prefix) ? "Hotils" : prefix;
        if (null == threadGroup) {
            threadGroup = ThreadPoolUtils.currentThreadGroup();
        }
