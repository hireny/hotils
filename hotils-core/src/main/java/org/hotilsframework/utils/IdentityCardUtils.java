...
import org.hotilsframework.core.lang.enums.Gender;
...
/**
 * @ClassName: IdCardNumberUtils
 * @Description: TODO   身份证号验证工具类
 * @Author: hireny
 * @Date: Created in 2020-01-09 15:17
 * @Version: 1.0
 */
public final class IdentityCardUtils {
     ...
     * @return
     */
    public static final boolean isIdentity(String number) {
        if (StringUtils.isBlank(number)) {
            return false;
        }
        number = number.trim();
        if (BLACK_SET.contains(number)) {
            return false;
        }
        if (!checkIdCardNumberRegex(number)) {
            return false;
        }
        if (!checkIdCardNumberArea(number.substring(0, 6))) {
            return false;
        }
        number = convertFifteenToEighteen(number);
        if (!checkBirthday(number.substring(6, 14))) {
            return false;
        }
        if (!checkIdCardNumberVerifyCode(number)) {
            return false;
        }
        return true;
    }
     ...
     */
    public static final Gender getGenderFromPersonIDCode(String identity) throws Throwable {
        String id = getIDCode(identity);
     ...
     * 身份证正则校验
     */
    private static boolean checkIdCardNumberRegex(String idNumber) {
     ...
     * 身份证地区码检查
     */
    private static boolean checkIdCardNumberArea(String idNumberArea) {
     ...
     * 身份证校验码检查
     */
    private static boolean checkIdCardNumberVerifyCode(String idNumber) {
